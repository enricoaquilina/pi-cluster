...
class TestFetchDeepSeekBalance:
    ...
    def test_returns_balance_on_success(self):
        from main import _fetch_deepseek_balance
        ...
        with patch("main.urlopen_", return_value=mock_response):
            result = _fetch_deepseek_balance.__wrapped__()
    ...
    def test_returns_error_on_failure(self):
        from main import _fetch_deepseek_balance
        with patch("main.urlopen_", side_effect=Exception("timeout")):
            result = _fetch_deepseek_balance.__wrapped__()
...
class TestFetchMoonshotBalance:
    ...
    def test_returns_balance_on_success(self):
        from main import _fetch_moonshot_balance
        mock_response = MagicMock()
        mock_response.read.return_value = json.dumps({
            "data": {
        ...
        mock_response.__exit__ = MagicMock(return_value=False)
        with patch("main.urlopen_", return_value=mock_response):
            result = _fetch_moonshot_balance.__wrapped__()
    ...
    def test_returns_error_on_failure(self):
        from main import _fetch_moonshot_balance
        with patch("main.urlopen_", side_effect=Exception("connection refused")):
            result = _fetch_moonshot_balance.__wrapped__()
        assert "error" in result
        assert result["provider"] == "moonshot"
...
class TestFetchTavilyUsage:
    ...
    def test_returns_usage_on_success(self):
        from main import _fetch_tavily_usage
        mock_response = MagicMock()
        mock_response.read.return_value = json.dumps({
        ...
        mock_response.__exit__ = MagicMock(return_value=False)
        with patch("main.urlopen_", return_value=mock_response):
            result = _fetch_tavily_usage.__wrapped__()
    ...
    def test_returns_error_on_failure(self):
        from main import _fetch_tavily_usage
        with patch("main.urlopen_", side_effect=Exception("401")):
            result = _fetch_tavily_usage.__wrapped__()
        assert "error" in result
        assert result["provider"] == "tavily"
...
class TestBillingAlerts:
    ...
    def test_low_balance_alert(self):
        ...
        from main import _check_balance_alert
    ...
    def test_high_usage_alert(self):
        ...
        from main import _check_usage_alert
