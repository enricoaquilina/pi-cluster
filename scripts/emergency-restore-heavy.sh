...
#   - Disables NFS server on master (heavy is primary)
#   - Re-points all node services to heavy
#   - Updates Cloudflare tunnel to heavy
#   - Syncs any data written to master during outage back to heavy
...
log "=== Restoring normal operation to heavy (192.168.0.5) ==="
...
# 2. Sync any data written to master during outage back to heavy
log "Syncing data written during outage from master..."
rsync -az --update master:/mnt/external/openclaw/workspace/ /mnt/data/openclaw/workspace/ 2>/dev/null || log "WARN: workspace sync from master failed"
rsync -az --update master:/mnt/external/polymarket-bot/data/ /mnt/data/polymarket-bot/data/ 2>/dev/null || log "WARN: polybot sync from master failed"

# 3. Stop emergency services on master
...
# 4. Disable NFS server on master (heavy is primary)
log "Disabling NFS server on master..."
ssh -o ConnectTimeout=5 master "sudo systemctl stop nfs-kernel-server && sudo systemctl disable nfs-kernel-server" 2>/dev/null || log "WARN: master NFS disable failed"

# 5. Verify heavy NFS is serving
...
showmount -e localhost >/dev/null 2>&1 && log "NFS: exporting" || log "WARN: NFS not exporting"
