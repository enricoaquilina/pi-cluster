...
for node_def in "${NODES[@]}"; do
    ...
    connected="false"
    if docker exec "$GATEWAY_CONTAINER" sh -c 'OPENCLAW_GATEWAY_TOKEN=REDACTED_GATEWAY_TOKEN timeout 10 node dist/index.js nodes status 2>&1' | grep -q "$name.*connected"; then
        connected="true"
    fi
