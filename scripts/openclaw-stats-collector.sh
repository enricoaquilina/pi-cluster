...
connected_raw=$(docker exec "$GATEWAY_CONTAINER" sh -c 'OPENCLAW_GATEWAY_TOKEN=dd697e7d788d7cb4995dc0d26778c80f18d6732b5c498c8a timeout 8 node dist/index.js nodes status 2>&1' | grep "paired.*connected" | grep -v "disconnected" | grep -oP '^\│\s*\K\S+' | tr -d '│ ' 2>/dev/null || echo "")
